...
for tc in range(1, T + 1):
    ...
    print('타겟 => ',target)
    # 데이터를 문자열로 결합 후 정수 변환하고 최솟값과 최댓값 초기화
    MIN = int(''.join(lst))
    MAX = int(''.join(lst))
    # 데이터 리스트에서 인덱스 2개를 선택하는 모든 조합에 대해 반복
    for value in combinations(target, 2): # 이것도 결국 완전탐색 이다
        ...
        # 스왑을 다시 원상복구 현재까지의 최대,최소가 정답이 아닐 수 있기 때문에 다시 원상태에서 평가해야한다.
    print(f'#{tc} {MIN} {MAX}')
